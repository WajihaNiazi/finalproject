...
<?php  
    ...
    if(!isset($_GET['updateID'])){
		header("location:bookingreport.php?Err_redirec=1");
		die;
    }
    ...
    $result = $db->select_one("room_booking_details","id='$id'");										
    $name=$result['name'];
    $email= $result['email'];
    $phone= $result['phone'];
    ...
    if(isset($_POST['submit'])){
        ...
        if(empty($name_Err) && empty($email_Err) && empty($phone_Err) && empty($address_Err) && empty($city_Err)&& empty($state_Err) && empty($zip_Err) && empty($contry_Err)&& empty($room_type_Err) && empty($check_in_date_Err)&& empty($check_out_date_Err) && empty($Occupancy_Err)){
            $data = sprintf("name='%s', email='%s', phone='%s', address='%s', city='%s', state='%s', zip='%d', contry='%s', room_type='%s', check_in_date='%s', check_in_time='%s', check_out_date='%s', Occupancy='%s'",
                mysqli_real_escape_string($GLOBALS["DB"],$name),
                mysqli_real_escape_string($GLOBALS["DB"],$email),
                mysqli_real_escape_string($GLOBALS["DB"],$phone),
                mysqli_real_escape_string($GLOBALS["DB"],$address),
                mysqli_real_escape_string($GLOBALS["DB"],$city),
                mysqli_real_escape_string($GLOBALS["DB"],$state),
                mysqli_real_escape_string($GLOBALS["DB"],$zip),
                mysqli_real_escape_string($GLOBALS["DB"],$contry),
                mysqli_real_escape_string($GLOBALS["DB"],$room_type),
                mysqli_real_escape_string($GLOBALS["DB"],$check_in_date),
                mysqli_real_escape_string($GLOBALS["DB"],$check_in_time),
                mysqli_real_escape_string($GLOBALS["DB"],$check_out_date),
                mysqli_real_escape_string($GLOBALS["DB"],$Occupancy)
            );
            $update = $db->update("room_booking_details",$data,"id='$id'");
            if($update){
                header("location:bookingupdate.php?update=1");
            }else{
                echo "Failed update";
            } 
        }else{
